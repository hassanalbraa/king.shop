...
export function AdminDashboard() {
  const { offers, updateBalance, updateOfferPrice, deleteOffer, logout } = useAppContext();
  const { toast } = useToast();
  const firestore = useFirestore();
  ...
  const [offerToDelete, setOfferToDelete] = useState<EditableOffer | null>(null);
            ...
            <Table>
              ...
              <TableBody>
                {editableOffers.map((offer) => (
                  <AlertDialog key={offer.id} onOpenChange={(open) => {if (!open) setOfferToDelete(null)}}>
                    <TableRow>
                      ...
                      <TableCell className="text-left flex gap-2 justify-end">
                        ...
                        <AlertDialogTrigger asChild>
                           <Button size="sm" variant="destructive" onClick={() => setOfferToDelete(offer)}>
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                            هل أنت متأكد من رغبتك في حذف عرض "{offer?.name} - {offer?.description}"؟ لا يمكن التراجع عن هذا الإجراء.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
